...
function BookedOrder({ order, cancelOrder, markReceived }) {
  ...
  return (
    <div className="order-card">
      ...
      {location.pathname == "/getorders" && !received && !cancelled && (
        <button className="order-button" onClick={deleteOrder}>
          Cancel
        </button>
      )}
      {location.pathname == "/vieworders" && !received && !cancelled && (
        <button className="order-button" onClick={isReceived}>
          Received
        </button>
      )}
    </div>
